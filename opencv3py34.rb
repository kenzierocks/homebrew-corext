...
class Opencv3py34 < Formula
  ...
  depends_on "python3@3.4.6" => :optional
  depends_on "qt5" => :optional
  depends_on "tbb" => :optional
  depends_on "vtk" => :optional

  with_python = build.with?("python") || build.with?("python3@3.4.6")
  pythons = build.with?("python3@3.4.6") ? ["with-python3@3.4.6"] : []
  ...
  def install
    ...
    args << "-DBUILD_opencv_python3=" + arg_switch("python3@3.4.6")
    ...
    if build.with? "python3@3.4.6"
      py3_config = `python3.4-config --configdir`.chomp
      py3_include = `python3.4 -c "import distutils.sysconfig as s; print(s.get_python_inc())"`.chomp
